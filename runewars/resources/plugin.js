...
function initialize() {
	...
	try {
		...
		Language.getInterface().addStrings( 'text/ui' );
		Language.getInterface().addStrings( 'text/common' );
		Language.getGame().addStrings( 'text/game' );
		Language.getGame().addStrings( 'text/common' );

		// create the Runewars named object
		useLibrary( 'res://namedobj.js' );
		const huntAndGather = Eons.namedObjects.runewars;

		// register Talisman as a game supported by Strange Eons
		Game.register('RW', @rw_game, #rw_game,	ImageUtils.get( 'plugin.png' ), template);

		// now that the game is registered, we can add the
		// default settings to use for new cards
		Game.get( 'RW' ).masterSettings.addSettingsFrom( 'base.settings' );

		ClassMap.add( 'base.classmap' );

	} catch( ex ) {
		Eons.log.severe( 'Runewars plug-in failed to start' );
